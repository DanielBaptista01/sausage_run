...
static void debugDraw(const Fase* f,const Scooby* s,const Maria* m,const Bola* b,const EventoSom* som,const RecursosMapa* r)
{
    ...
    Retangulo hs=hitboxPersonagem(&s->corpo,s->corpo.x,s->corpo.y),hm=hitboxPersonagem(&m->corpo,m->corpo.x,m->corpo.y);
    caixa(hs,al_map_rgb(70,255,95),2);caixa(hm,al_map_rgb(255,235,60),2);
    ...
    if(r&&r->fonte)
    {
        char txt[256];snprintf(txt,sizeof(txt),"IA: %s | caminho %d/%d | stuck %.2fs (%d) | bola %s",
            nomeEstadoMaria(m->estado),m->indiceCaminho,m->quantidadeCaminho,m->tempoSemProgresso,m->eventosAntiStuck,b->coletada?"coletada":"no mapa");
        al_draw_filled_rectangle(450,12,1260,39,al_map_rgba(0,0,0,190));
        al_draw_text(r->fonte,al_map_rgb(255,255,255),462,20,0,txt);
    }
}
