...
int main(void)
{
    ...
    {
        printf("Erro ao iniciar allegro_primitives.\n");
        return -1;
    }

    if (!al_init_image_addon())
    {
        printf("Erro ao iniciar allegro_image.\n");
    ...
    if (audioInstalado)
    {
        ...
        {
            printf(
                "Aviso: nao foi possivel reservar vozes de audio. O jogo continuara sem som.\n");
    ...
    {
        printf(
            "Aviso: audio nao disponivel. O jogo continuara sem som.\n");
    }
    ...
    {
        printf(
            "Erro ao criar display/timer/fila.\n");
    ...
    ALLEGRO_FONT* fonteCarregamento =
    ...
    scooby.corpo.largura = 46.0f;
    scooby.corpo.altura = 30.0f;
    scooby.corpo.velocidade = 135.0f;
    scooby.direcaoSprite = DIRECAO_UP;

    maria.corpo.largura = 34.0f;
    maria.corpo.altura = 42.0f;
    ...
    {
        printf(
            "Nao foi possivel carregar todos os recursos obrigatorios.\n");
    ...
    while (executando)
    {
            ...
            ALLEGRO_EVENT_KEY_DOWN)
        {
                 ...
                 estado == JOGO_GAME_OVER))
            {
                ...
                if (!carregarRecursosFase(
                        &recursos,
                        fases,
                        faseAtual))
                {
                    printf(
                        "Erro ao recarregar fase inicial.\n");
            ...
            ALLEGRO_EVENT_TIMER)
        {
                     ...
                     JOGO_TRANSICAO_FASE)
            {
                    ...
                    TRANSICAO_FADE_IN)
                {
                    ...
                    if (transicao.tempo > 0.30f)
                    {
                        transicao.etapa =
                            TRANSICAO_FADE_OUT;

                        float t =
                            (transicao.tempo - 0.30f) /
                            0.90f;
                    ...
                    if (transicao.tempo >= 1.20f &&
                        !transicao.faseTrocada)
                    {
