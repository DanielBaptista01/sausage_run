...
bool mariaOuveSom(const Maria* m, const EventoSom* s)
{
    ...
    if (alcance > m->alcanceAudicao)
        alcance = m->alcanceAudicao;
...
bool mariaVeScooby(const Maria* m, const Scooby* s, const Fase* f)
{
    ...
    if (distancia(m->corpo.x, m->corpo.y,
                  s->corpo.x, s->corpo.y) > m->alcanceVisao)
    {
        return false;
    }
    ...
    if (fabsf(normalizarAngulo(angulo - m->corpo.direcao)) >
        m->anguloVisao * 0.5f)
    {
        return false;
    }

    return linhaVisaoLivre(
        f,
        m->corpo.x, m->corpo.y,
        s->corpo.x, s->corpo.y);
}
...
bool mariaPodeCapturar(const Maria* m, const Scooby* s, const Fase* f)
{
    ...
    float dx = fabsf(m->corpo.x - s->corpo.x) -
               (m->corpo.largura + s->corpo.largura) * 0.5f;

    float dy = fabsf(m->corpo.y - s->corpo.y) -
               (m->corpo.altura + s->corpo.altura) * 0.5f;

    if (dx < 0.0f) dx = 0.0f;
    if (dy < 0.0f) dy = 0.0f;

    float distanciaHitbox = sqrtf(dx * dx + dy * dy);

    if (distanciaHitbox > 14.0f)
        return false;
...
static bool celulaBloqueada(const Maria* m, const Fase* f, int c, int r)
{
    ...
    if (!personagemDentroArea(&m->corpo, p.x, p.y, f))
        return true;

    float margemX = m->corpo.largura * 0.5f;
    float margemY = m->corpo.altura * 0.5f;
    ...
    for (int i = 0; i < f->quantidadeObstaculos; i++)
    {
        ...
        if (p.x >= o->x - margemX &&
            p.x <= o->x + o->largura + margemX &&
            p.y >= o->y - margemY &&
            p.y <= o->y + o->altura + margemY)
        {
            return true;
        }
    }
...
static void destinoLivre(const Maria* m, const Fase* f, int* tc, int* tr)
{
    if (!m || !f || !tc || !tr) return;
    if (!celulaBloqueada(m, f, *tc, *tr)) return;
                            ...
                            float destinoX, float destinoY)
{
    ...
    if (sc == tc && sr == tr)
    {
        Ponto alvo = destinoOriginalLivre ?
            (Ponto){ destinoX, destinoY } :
            centroCelula(tc, tr);
        ...
        m->tempoRecalcularCaminho = 0.28f;
        return true;
    }
    ...
    int filaC[GRID_TOTAL];
    int filaR[GRID_TOTAL];
                                        ...
                                        int frameAnterior, bool correndo)
{
    ...
    if (a->frameAtual == frameAnterior)
        return;

    if (a->frameAtual != 1 && a->frameAtual != 3)
        return;

    if (!audio) return;

    if (correndo)
        tocarEfeitoPosicional(audio->mariaCorrida, m->corpo.x, 0.28f);
    else
        tocarEfeitoPosicional(audio->mariaPasso, m->corpo.x, 0.20f);
}
                       ...
                       float x, float y, float dt, float multiplicador)
{
    ...
    if (m->tempoRecalcularCaminho <= 0.0f ||
        m->indiceCaminho >= m->quantidadeCaminho ||
        alvoMudou)
    {
        calcularCaminho(m, f, x, y);
    }

    if (m->indiceCaminho >= m->quantidadeCaminho)
        return;
    ...
    m->direcaoSprite =
        direcaoSpritePorMovimento(dx, dy, m->direcaoSprite);
...
static void escolherAlvoBusca(Maria* m, const Fase* f)
{
    for (int tentativa = 0; tentativa < 12; tentativa++)
    {
        ...
        float raio = 55.0f + (float)(rand() % 120);

        float x = m->ultimaPosicaoVistaX + cosf(angulo) * raio;
        float y = m->ultimaPosicaoVistaY + sinf(angulo) * raio;

        if (!personagemDentroArea(&m->corpo, x, y, f))
            continue;

        if (!celulaBloqueada(m, f, gridCol(x), gridRow(y)))
        {
            m->alvoX = x;
            m->alvoY = y;
                    ...
                    const Fase* f, RecursosAudio* audio, float dt)
{
    ...
    {
        if (atualizarAnimacaoUmaVez(&m->pick, dt))
            m->capturaConcluida = true;
        return;
    }
    ...
    switch (m->estado)
    {
        case MARIA_PATRULHA:
        {
            ...
            {
                m->waypointAtual =
                    (m->waypointAtual + 1) % f->quantidadeWaypoints;
                m->tempoRecalcularCaminho = 0.0f;
            }
            break;
        }
        ...
        case MARIA_PROCURAR:
            ...
            {
                if (m->tempoNovoAlvoBusca <= 0.0f ||
                    chegouAlvoNavegavel(m))
                {
    ...
    if (mariaPodeCapturar(m, s, f))
    {
        ...
        if (audio)
            tocarEfeitoPosicional(audio->captura, m->corpo.x, 0.78f);
    ...
    if (m->estado == MARIA_PERSEGUIR)
    {
        ...
        if (m->movendo)
            tocarPassoMariaSeNecessario(m, audio, frameAnterior, true);
    }
    else if (m->movendo)
    {
